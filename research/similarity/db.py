...
class Regexes(EntityMeta):
    ...
    prototype = Column(INTEGER, ForeignKey(f'{REGEXES_TABLE_NAME}.id'), nullable=True)
    # extra fields
...
class Results(EntityMeta):
    ...
    id = Column(INTEGER, primary_key=True, index=True, autoincrement=True)
    # core fields
    regex1 = Column(INTEGER, ForeignKey(f'{REGEXES_TABLE_NAME}.id'), nullable=False)
    regex2 = Column(INTEGER, ForeignKey(f'{REGEXES_TABLE_NAME}.id'), nullable=False)
    regex_type = Column(TEXT, nullable=False)
